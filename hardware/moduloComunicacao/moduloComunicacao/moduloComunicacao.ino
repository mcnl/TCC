...
void loop() {
  pegarLocaldoFirebase("latitude");
  pegarLocaldoFirebase("longitude");
}

int pegarDatadoFirebase(String unidadeTempo){
  FirebaseData retornoFirebase;
  if (Firebase.getInt(retornoFirebase, "/device-1/data/"+unidadeTempo)) {
    if (retornoFirebase.dataType() == "int") {
      return retornoFirebase.intData();
    }
  } 
  else {
    Serial.println(retornoFirebase.errorReason());
    return false;
  }
  delay(2000);
}

double pegarLocaldoFirebase(String unidadeLocal){
  FirebaseData retornoFirebase;
  if (Firebase.getDouble(retornoFirebase, "/device-1/local/"+unidadeLocal)) {
    if (retornoFirebase.dataType() == "double") {
      return retornoFirebase.doubleData();
    }
  } 
  else {
    Serial.println(retornoFirebase.errorReason());
